...
| **決賽（6/26）** | **必須實作 API 串接 + 完整介面 demo**（從 [2026 初賽提案簡報範例.pdf](../參考資料/2026%20初賽提案簡報範例.pdf) 第 30 行明示） |
...
從 [2026 初賽提案簡報範例.pdf](../參考資料/2026%20初賽提案簡報範例.pdf) 看，主辦方示範題目是「**颱風百問智慧小幫手**」。它的特色：

| 主辦範例特色 | 我們的對應 |
| --- | --- |
| 4 種受眾（政府、企業、民眾、學術） | 我們 2 種（人資 + 勞工/工會），但**雙面介面 = 整合創新** |
| 結構化（颱風列表）+ 非結構化（颱風百問） | 我們 29 PDF + 13 節點 + 13 關係 + 26,513 筆裁罰資料層 |
...
1. **資料量大但敘事容易分散**
   - 目前有 29 PDF、13 節點、13 關係、26,513 筆裁罰資料層
   - 風險不是資料不足，而是評審看不懂資料如何支撐 demo
   - **緩解**：簡報用「資料清理與匯入 → 知識圖繪製 → 設定節點與關係 → Demo Q&A」四步驟呈現
...
**Day 6（5/9）：寫 PPT**
- 套用 [2026 初賽提案簡報範例.pdf](../參考資料/2026%20初賽提案簡報範例.pdf) 結構
...
| --- | --- | --- |
| 1 | 封面：題目、隊名、slogan | — |
| 2 | 痛點：企業職災發生後的 8 小時、罰鍰、訴訟與治理壓力 | 25% 痛點 |
| 3 | 解決方案：B2B 合規風險主軸 + 勞工權益雙面介面 | 25% 痛點 |
| 4 | 資料來源：29 PDF + 26,513 筆裁罰 + 13 節點 / 13 關係 | 30% 技術 |
| 5 | Gemini Data 平台操作 1：資料清理與匯入 | 30% 技術 |
| 6 | Gemini Data 平台操作 2：知識圖繪製 | 30% 技術 |
| 7 | Gemini Data 平台操作 3：設定節點與關係（Category + Unique Key） | 30% 技術 |
| 8 | Hybrid RAG 架構：VectorRAG + GraphRAG + Risk Layer | 30% 技術 |
| 9 | Context Engineering / Robot Setting | 30% 技術 |
| 10 | Demo 1：工地墜落重大職災三軌風險 | 20% 實作 |
| 11 | Demo 2：同業裁罰與產業風險 | 20% 實作 |
| 12 | Demo 3：同一事故的人資端 vs 勞工端 | 20% 實作 |
| 13 | 介面初步構想：身分切換、對話、推理路徑、倒數計時 | 10% 介面 |
| 14 | 商業價值：中小企業、人資顧問、法遵服務、工會/法扶 | 10% 商業 |
| 15 | 結論：為何必須用 Gemini Data Hybrid RAG | — |
